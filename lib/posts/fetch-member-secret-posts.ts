...
export async function fetchMemberSecretPosts(): Promise<PublishedPost[]> {
  ...
  let { data, error } = await supabase
    .from("posts")
    .select(
      "id, tipo, slug, titulo, descricao, media_url, preco_desde, preco_base_eur, has_variants, link_cta, data_publicacao, data_fim_publicacao, ordem_site, membros_apenas, feed_vibe_slugs, hover_line, pets_allowed, capacidade_min, capacidade_max, post_hotels(nome, regime, status)",
    )
